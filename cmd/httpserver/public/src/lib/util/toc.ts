 ...
 * @returns {VNode[]}
 */
function wrapChildren<T>(wrapper: T, elements: HeadingTree[], depth = 0): VNode[] {
  if (!elements) return []
  ...
  return elements.map((el) => {
    return h(
      ...
      () => wrapChildren(wrapper, el.children, depth)
    )
  })
}
...
function recurseNodes(root: Node, tree: HeadingTree[] = []): HeadingTree[] {
  ...
  if (root.childNodes === undefined) return reorder(tree)
