...
// WithUserGuilds tells the query-builder to eager-load the nodes that are connected to
// the "user_guilds" edge. The optional arguments are used to configure the query builder of the edge.
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withUserGuilds; query != nil {
		if err := uq.loadUserGuilds(ctx, query, nodes,
			func(n *User) { n.Edges.UserGuilds = []*Guild{} },
			func(n *User, e *Guild) { n.Edges.UserGuilds = append(n.Edges.UserGuilds, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadUserGuilds(ctx context.Context, query *GuildQuery, nodes []*User, init func(*User), assign func(*User, *Guild)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Where(sql.InValues(joinT.C(user.UserGuildsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.UserGuildsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "user_guilds" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
// WithNamedUserGuilds tells the query-builder to eager-load the nodes that are connected to the "user_guilds"
